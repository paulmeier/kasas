...
// rulesView is the Rules page: a list of auto-labeling rules and a create/edit
// form. A rule pairs a condition (a query in the kasas search syntax, validated
// in-browser for instant feedback) with the labels applied to every matching
// transaction. Enabled rules auto-apply to newly-synced transactions; "Run"
// applies a rule (or all enabled rules) over existing transactions.
...
func (v *rulesView) onSave(ctx app.Context, _ app.Event) {
	...
	if len(v.formLabels) == 0 {
		v.formErr = "Add at least one label to apply."
		ctx.Update()
		return
	}
...
func (v *rulesView) onToggleEnabled(ctx app.Context, r rule) {
	id := r.ID
	payload := rulePayload{Name: r.Name, Query: r.Query, Labels: r.Labels, Enabled: !r.Enabled}
	ctx.Async(func() {
...
func (v *rulesView) onRun(ctx app.Context, r rule) {
	...
	ctx.Async(func() {
		...
		ctx.Dispatch(func(ctx app.Context) {
			...
			v.runMsg = fmt.Sprintf("Ran %q: labeled %d of %d matching transactions.", name, res.Updated, res.Matched)
			ctx.Update()
		})
	})
}
...
func (v *rulesView) onRunAll(ctx app.Context, _ app.Event) {
	...
	ctx.Async(func() {
		...
		ctx.Dispatch(func(ctx app.Context) {
			...
			v.runMsg = fmt.Sprintf("Ran all enabled rules: labeled %d of %d matching transactions.", res.Updated, res.Matched)
			ctx.Update()
		})
	})
}
...
func (v *rulesView) onDelete(ctx app.Context, r rule) {
	msg := fmt.Sprintf("Delete the rule %q? Labels already applied to transactions are kept.", ruleLabel(r))
...
func (v *rulesView) Render() app.UI {
	return v.renderShell(navRules,
		app.Header().Class("page-header").Body(
			...
			app.Span().Class("page-subtitle").Text("Automatically label transactions that match a query"),
		),
		v.renderError(),
		v.renderToolbar(),
		v.renderForm(),
...
func (v *rulesView) renderFormLabels() app.UI {
	if len(v.formLabels) == 0 {
		return app.Div().Class("rule-form-labels").Body(
			app.Span().Class("empty-hint").Text("No labels yet — add at least one."),
		)
	}
	...
	if len(v.rules) == 0 {
		return app.Div().Class("empty-state").Body(
			...
			app.P().Class("empty-hint").Text(
				`Click “New rule” to create one — for example, label everything from a payee, or flag large charges for review.`),
		)
	}
...
func (v *rulesView) renderRuleRow(r rule) app.UI {
	return app.Tr().Body(
		...
		app.Td().Body(renderRuleLabelChips(r.Labels)),
