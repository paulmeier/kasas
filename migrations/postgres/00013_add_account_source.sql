...
-- row.
-- text (not an enum) keeps the generated struct byte-identical to the SQLite
-- backend's, which the pgstore adapter relies on. The default backfills every
-- existing row correctly (all current accounts are SimpleFIN). Source is never
-- written by UpsertAccount's conflict-update path, so re-syncs leave it intact:
-- the lineage is immutable, like labels and extensions.
